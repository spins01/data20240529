...
import static com.spins.intech.sipsample.service.PortSipService.EXTRA_CALL_SEESIONID;
    ...
    public void onClick(View view) {
        if (application.getMEngine() != null) {
            ...
            if (view.getId() == R.id.answer_audio || view.getId() == R.id.answer_video) {
                ...
                int ret = application.getMEngine().answerCall(sessionId, view.getId() == R.id.answer_video);
                if (ret != 0) {
                    Toast.makeText(this, "answerCall Failed! ret=" + ret, Toast.LENGTH_SHORT);
                }
                if (application.getMConference()) {
                    application.getMEngine().joinToConference(currentLine.sessionID);
                }
            } else if (view.getId() == R.id.hangup_call) {
                Ring.getInstance(this).stop();
                if (currentLine.state == Session.CALL_STATE_FLAG.INCOMING) {
                    application.getMEngine().rejectCall(currentLine.sessionID, 486);
                    currentLine.Reset();

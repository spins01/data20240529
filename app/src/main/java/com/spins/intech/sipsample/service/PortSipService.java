package com.spins.intech.sipsample.service;
...
import com.spins.intech.sipsample.ui.IncomingActivity;
import com.spins.intech.sipsample.ui.MainActivity;
import com.spins.intech.sipsample.util.CallManager;
import com.spins.intech.sipsample.util.Contact;
import com.spins.intech.sipsample.util.ContactManager;
import com.spins.intech.sipsample.util.NetWorkReceiver;
...
//        }
    ...
    public void onInviteIncoming(long sessionId,
        ...
        if(CallManager.Instance().findIncomingCall()!=null){
            applicaton.getMEngine().rejectCall(sessionId,486);//busy
            return;
        }
    ...
    public void onInviteConnected(long sessionId) {
        ...
        if (session != null) {
            ...
            if (applicaton.getMConference())
            {
                applicaton.getMEngine().joinToConference(session.sessionID);
                applicaton.getMEngine().sendVideo(session.sessionID, true);
            }
    ...
    public void onPresenceRecvSubscribe(
        ...
        switch (contact.state) {
            case ACCEPTED://This subscribe has accepted
                applicaton.getMEngine().presenceAcceptSubscribe(subscribeId);
                break;
            case REJECTED://This subscribe has rejected
                applicaton.getMEngine().presenceRejectSubscribe(subscribeId);
                break;
            case UNSETTLLED:
                break;
            case UNSUBSCRIBE:
    ...
    public void onNetworkChange(int netMobile) {
        if(netMobile==-1){
            ...
            if(CallManager.Instance().online) {
                applicaton.getMEngine().refreshRegistration(0);
            }else{
                //
            }
        }
